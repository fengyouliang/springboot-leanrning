...
| 实例 vs 代理（最终暴露对象） | “为什么注入的是 proxy？”“谁把对象换掉了？” | `AbstractAutowireCapableBeanFactory#initializeBean`、`AbstractAutowireCapableBeanFactory#applyBeanPostProcessorsAfterInitialization` | `beanName`、`bean` vs `result`、`beanFactory.getBeanPostProcessors()` | [06](../part-01-ioc-container/017-06-post-processors.md)、[31](../part-04-wiring-and-boundaries/31-proxying-phase-bpp-wraps-bean.md)、`SpringCoreBeansBeanCreationTraceLabTest` |
