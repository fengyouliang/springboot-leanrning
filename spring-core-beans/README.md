...
| Boot 自动装配：条件报告/顺序/退让时机 | [docs/10](docs/10-spring-boot-auto-configuration.md) + [docs/11](docs/11-debugging-and-observability.md) | `src/test/java/.../SpringCoreBeansConditionEvaluationReportLabTest.java` + `src/test/java/.../SpringCoreBeansAutoConfigurationOrderingLabTest.java` + `src/test/java/.../SpringCoreBeansAutoConfigurationBackoffTimingLabTest.java` | 你能解释“为什么 match/why skip”、为什么“运行时有 bean 但条件仍不生效”、为什么“写了覆盖 Bean 但没退让”，并能用 after/before/时机把行为确定化 |
