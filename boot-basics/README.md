...
```bash
mvn -pl boot-basics spring-boot:run -Dspring-boot.run.profiles=dev
```
...
启用 Exercises：打开 `*ExerciseTest`，移除/注释 `@Disabled`，按提示完成后再运行 `mvn -pl boot-basics test`。
