# 第 33 章：04. `final` 与代理限制：为什么 final method 拦截不到？
<!-- CHAPTER-CARD:START -->
!!! summary "章节学习卡片（五问闭环）"
...
上一章：[第 32 章：03. 自调用（self-invocation）：为什么 `this.inner()` 不会被拦截？](032-03-self-invocation.md) ｜ 全书目录：[Book TOC](/book/) ｜ 下一章：[第 34 章：05. exposeProxy：用 `AopContext.currentProxy()` 绕过自调用（进阶）](034-05-expose-proxy.md)
<!-- GLOBAL-BOOK-NAV:END -->
...
4. **同类内部调用（self-invocation）**
   - 不是语言限制，但效果类似：不走 proxy 就不拦截（见 [03. self-invocation](032-03-self-invocation.md)）
