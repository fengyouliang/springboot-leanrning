...
> 对应 beans 深挖章节：
> - `spring-core-beans/docs/15-pre-instantiation-short-circuit.md`
> - `spring-core-beans/docs/16-early-reference-and-circular.md`
...
- `DefaultAopProxyFactory#createAopProxy`（JDK vs CGLIB 选择点，配合 docs/02）
...
1. **调用是否走 proxy？**（见 docs/01：call path）
2. **候选 Advisors 是否存在？**（看看容器里是否真的注册了 advisor）
3. **pointcut 是否对目标类/方法适用？**（见 docs/08：pointcut 系统）
4. **是否踩中代理限制？**（docs/02、docs/04：JDK/CGLIB、final/self-invocation）
